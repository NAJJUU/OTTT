		    ...
		    <h1 style="font-size: 21px; display: inline-block;">회원가입</h1>
		        ...
		        let code
				    ...
				    $(".send-verification-code").on("click", function() {
				        ...
				        const checkInput = $("#Certification"); // 인증번호 입력하는 곳
				        var data = { email: email };
				
				        $.ajax({
				            type: 'get',
				            url: 'ottt/signin/mailCheck?email=' + email, //get 방식이라 url뒤에 email을 븥일 수 있다.
				            data: data,
				            success: function(result) {
				                console.log('result: ' + result);
				                checkInput.prop("disabled", false); // 속성 값 설정 수정
				                code = result;
				                $(".body").html("인증번호가 전송되었습니다.");
				                $('#Modal').modal('show');
				            }
				        }); // end ajax
				    }); //end send eamil
				    ...
				    $("#completion").on("click", function() {
				        ...
				        if (inputCode === code) {
				            ...
				            $('#Modal').modal('show');
				            $("#send-verification-code").attr('disabled', true);
				        } else {
		          	...
		          	<div class="email">
		           		...
		           		<button id="send-verification-code" class="send-verification-code" type="button">인증번호 발송</button>
           			</div>
           			
		          	<div class="Certification">
		          		<input type="text" id="Certification" title="EM" maxlength="6" disabled="disabled"  placeholder="인증번호" >
